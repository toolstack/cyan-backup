<?php
	...
	if( isset( $_POST['remove_backup'] ) ) {
		...
		if( isset( $_POST['remove'] ) ) {
			...
			foreach( (array)$_POST['remove'] as $index => $bfile ) {
				...
				if( isset( $postdata['remove'][$index] ) ) { 
				...
				if( $file !== FALSE) {
					...
					if( @unlink( $file ) === FALSE ) {
						$notes[] = "<strong>" . sprintf( __( 'ERROR: Failed to delete backup file: %s', 'cyan-backup' ), $file ) . "</strong>";
					}
	...
	$nonces = wp_nonce_field( $nonce_field, self::NONCE_NAME, true, false );
?>
<div class="wrap">

	<div id="icon-options-cyan-backup" class="icon32"><br /></div>

	<h2><?php _e( 'CYAN Backup', 'cyan-backup' );?></h2>

	<h3><?php _e( 'Run Backup', 'cyan-backup' );?></h3>

	<form method="post" id="backup_site" action="<?php echo $this->admin_action; ?>">
		<?php echo $nonces; ?>
		<input type="hidden" name="backup_site" class="button-primary sites_backup" value="<?php _e( 'Backup Now!', 'cyan-backup' )?>" class="button" style="margin-left:1em;" />
		<p style="margin-top:1em" id='img_wrap'>
			<input type="submit" name="backup_site" class="button-primary sites_backup" value="<?php _e( 'Backup Now!', 'cyan-backup' )?>" class="button" style="margin-left:1em;" />
		</p>
	</form>
	...
	<h3><?php _e( 'Backup Files', 'cyan-backup' );?></h3>
	...
	<form method="post" action="<?php echo $this->admin_action; ?>">
		...
		<table id="backuplist" class="wp-list-table widefat fixed" style="margin-top:0;">
			...
			<thead>
				<tr>
					<th style="width: 40%;"><?php _e( 'File Name', 'cyan-backup' );?></th>
					<th style="width: 35%;"><?php _e( 'Date and Time', 'cyan-backup' );?></th>
					<th style="width: 15%;"><?php _e( 'Size', 'cyan-backup' );?></th>
			...
			<tfoot>
				<tr>
					<th colspan="3"></th>
					<th style="width: 75px; text-align: center;"><input type="submit" name="remove_backup" class="button-primary" value="<?php _e( 'Delete', 'cyan-backup' );?>" class="button" /></th>
				</tr>
			</tfoot>
