	...
	$help_screen->add_help_tab(
		array(
			...
			'content'  => 	'<p>' . __('There are multiple protocol providers available for remote storage, but only those that your system supports will be available in the protocol drop down.', $this->textdomain) . '</p>' .
							...
							'<p>' . __('<b>FTPS Library</b>:  FTPS Library use the PHP Library to transfer the backups.  FTPS is FTP over SSL/TLS.  If your installation of PHP does not have the the FTPS library installed you should use SFTP Wrappers instead.', $this->textdomain) . '</p>' .
							'<p>' . __('<b>SFTP/SCP Wrappers</b>:  SFTP/SCP Wrappers use the built in wrappers code to transfer the backups.  SFTP/SCP is Secure File Transfer over SSH.  If your hosting provider has disabled wrappers this will not work and you should use SFTP/SCP Library instead.', $this->textdomain) . '</p>' .
							'<p>' . __('<b>SFTP/SCP Library</b>:  SFTP/SCP Library use the PHP Library to transfer the backups.  SFTP/SCP is Secure File Transfer over SSH.  If your installation of PHP does not have the the SSH2 library installed you should use SFTP/SCP Wrappers instead.', $this->textdomain) . '</p>'
			,
			'callback' => 	false
		)
	);
